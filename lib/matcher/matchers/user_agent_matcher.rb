...
class UserAgentMatcher < GenericMatcher
  ...
  def execute
    ...
    [:'REQUEST_PATH',:'ORIGINAL_FULLPATH',:'HTTP_COOKIE', :'PATH_INFO',:'HTTP_USER_AGENT',:'SERVER_SOFTWARE',:'HTTP_X_REQUESTED_WITH',:'HTTP_X_CSRF_TOKEN',:'HTTP_IF_NONE_MATCH'].each do |key|
      data.delete(key)
    end
    {
